...
const UPLOAD_URL = process.env.UPLOAD_URL || '';      //节点或订阅自动上传地址,需填写部署Merge-sub项目后的首页地址,例如:/merge.xxx.com
const PROJECT_URL = process.env.PROJECT_URL || '';    // 需要上传订阅或保活时需要填写项目分配的url,例如:https://google.com
const AUTO_ACCESS = process.env.AUTO_ACCESS || false; // false关闭自动保活,true开启,需同时填写ROJECT_URL变量
const FILE_PATH = process.env.FILE_PATH || './tmp';   //运行目录 ,sub节点文件保存目录
const SUB_PATH = process.env.SUB_PATH || 'sub1216';       // 订阅路径
const PORT = process.env.SERVER_PORT || process.env.PORT || 3000;        // http服务订阅端口
const UUID = process.env.UUID || 'ff01fe27-e7d6-405b-90dc-9970c5777636'; // 使用哪吒v1,在不同的平台运行需修改UUID,否则会覆盖
const NEZHA_SERVER = process.env.NEZHA_SERVER || '';        //哪吒V1填写形式: 哪吒V0填写形式nz.abc.com:8008  z.abc.com
const NEZHA_PORT = process.env.NEZHA_PORT || '';            // 使用哪吒V1请留空,哪吒V0需赶写
const NEZHA_KEY = process.env.NEZHA_KEY || '';              //哪吒V1的NZ_CLIENT_SECRET或哪吒V0的agent密钥
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || '';          //固定隧道域名,留空即启用临时隧道
const ARGO_AUTH = process.env.ARGO_AUTH || '';              //固定隧道密钥,json或token,留空即启用临时隧道.json获取地址:https://json.zone.id
const ARGO_PORT = process.env.ARGO_PORT || 8001;            // 固定隧道端口,使用token,需在Cloudflare后台设置和这里一致
